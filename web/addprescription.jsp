...
<html>
    ...
    <body>
        ...
        <div class="main">
            <form action=# method="POST" onsubmit="return validate()">
                ...
                <div class="cmn"> <input class="docid" type="text" name="docid" id="did" required> </div> <span id="errorD" style="margin-left:220px;"></span>
                ...
                <div class="cmn"> 
                        ...
                        <input class="radio-one" type="radio" name="beds" value="Yes" /> 
    ...
    try {
        ...
        if (request.getParameter("btnInsert") != null) {
            ...
            if (x > 0 && y > 0 && z > 0) {
                /*if(p_bed == true){
                    ResultSet rst = stmt.executeQuery("select p_id from transaction_information where p_id like'" + p_id +"'");
                    if(rst.next()){
                        PreparedStatement psb1 = conn.prepareStatement("update transaction_information set fees=fees+100 where p_id like'" + p_id +"'");
                        psb1.executeUpdate();
                    }
                    else{
                        ...
                        psb2.setInt(2, 0);
                        PreparedStatement psb3 = conn.prepareStatement("update transaction_information set fees=fees+100 where p_id like'" + p_id +"'");
                        psb3.executeUpdate();
                    }
                    
                }*/
                ResultSet rs = stmt.executeQuery("select count(p_bed) from prescription where p_bed=true");
                rs.next();
                int count = rs.getInt(1);
                if(count<25)
                    out.print("<script>alert('Prescription Added Successfully.')</script>");
                else
                    out.print("<script>alert('Bed not available currently.')</script>");
            }
        }
        } catch (Exception ex) {
        ex.printStackTrace();
